            ...
            <div class="patientInfo">
                <div class="patientImg">
                    ...
                    <div class="info" v-if="getstaffSummary">
                        ...
                        <p class="overflow-wrap" :title="getstaffSummary?.email"><a :href="`mailto:${getstaffSummary?.email}`">
                                <MailOutlined /> {{ getstaffSummary?.email?getstaffSummary?.email:'' }}</a></p>
                        <p>
                ...
                <div class="pat-profile" v-if="getstaffSummary">
                    <div class="pat-profile-inner">
                      <div class="thumb-head">Client(s)</div>
                      <div class="thumb-desc">{{ getstaffSummary.client != "" ? getstaffSummary.client : 'Virtare' }}</div>
                    </div>
                    <!-- <div class="pat-profile-inner">
                    ...
                    <div class="pat-profile-inner">
                        ...
                        <div class="thumb-desc">{{getstaffSummary.role}}</div>
                    </div>
                    <!-- <div class="pat-profile-inner" v-if="getstaffSummary?.role?.length>0">
                        <div class="thumb-head">Role</div>
                        <a-tooltip placement="bottom">
                          <template #title>
                            <span>{{ getstaffSummary?.role }}</span>
                          </template>
                    ...
                    <div class="pat-profile-inner">
                        ...
                        <div class="thumb-desc">{{ getstaffSummary?getstaffSummary.status ? 'Active' : 'Inactive' : '' }}</div>
                    </div>
                    <!-- <div class="pat-profile-inner">
        ...
        <a-col :sm="17" :xs="24">
            ...
            <div class="summary-tabs">
                <a-tabs v-model:activeKey="activeKey1">
                    <a-tab-pane key="1" tab="Appointments" v-if="rolesPermissionFind(screensPermissions,[42])">
                      <AppointmentTable v-if="activeKey1=='1'" />
                    </a-tab-pane>
                    <a-tab-pane key="2" tab="Patients" v-if="rolesPermissionFind(screensPermissions,[43])"> 
                        <!-- PatientTable Table -->
                        <PatientTable v-if="activeKey1=='2'" />
                        <Loader />
                    </a-tab-pane>
                    <!-- <a-tab-pane key="3" tab="Availability" v-if="rolesPermissionFind(screensPermissions,[50])">
                        <a-row :gutter="24" v-if="activeKey1=='3'">
                            <a-col :span="24">
                                <AvailabilityTable :columnData="availabilityListColms" v-if="activeKey1=='3'"/>
                                <Loader />
                            </a-col>
                        </a-row>
                    </a-tab-pane>
                    <a-tab-pane key="4" tab="Emergency Contacts" v-if="rolesPermissionFind(screensPermissions,[46])">
                        <a-row :gutter="24" v-if="activeKey1=='4'">
                            <a-col :span="24">
                                <ContactTable :columnData="staffContactColms" v-if="activeKey1=='4'"/>
                                <Loader />
                            </a-col>
                        </a-row>
                    </a-tab-pane>
                    <a-tab-pane key="6" tab="Documents" v-if="rolesPermissionFind(screensPermissions,[45])">
                        <a-col :sm="24" :xs="24" v-if="activeKey1=='6'">
                        </a-col>
                        <a-row :gutter="24" v-if="activeKey1=='6'">
                            <a-col :sm="24" :xs="24">
                                <DocumentTable :columnData="documentColumns" v-if="activeKey1=='6'"/>
                                <Loader />
                            </a-col>
                        </a-row>
                    </a-tab-pane> -->
                    <a-tab-pane key="5" tab="Roles" v-if="role.user.staff.id!=paramId && (arrayToObjact(screensPermissions,54) || arrayToObjact(screensPermissions,55))">
                      <a-row :gutter="24" v-if="activeKey1=='5'">
                          <a-col :sm="24" :xs="24">
                            <div class="common-btn mb-24">
                              <svg width="34" height="34" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" @click="showModalRole" v-if="arrayToObjact(screensPermissions,55)" style="cursor: pointer;">
                          ...
                          <a-col :span="24">
                            <!-- Roles Table -->
                            <RoleTable v-if="arrayToObjact(screensPermissions,54)" />
                            <Loader />
                          </a-col>
                        </a-row>
                      </a-tab-pane>
                    <!-- <a-tab-pane key="9" tab="Locations" v-if="role.user.staff.id!=paramId && (arrayToObjact(screensPermissions, 479) || arrayToObjact(screensPermissions, 480))">
                        <a-row :gutter="24" v-if="activeKey1=='9'">
                            ...
                            <a-col :sm="24" :xs="24" v-if="arrayToObjact(screensPermissions, 479)">
                                <StaffLocationsTable :hideAction="false"/>
                                <Loader />
                            </a-col>
                        </a-row>
                    </a-tab-pane> -->
                    <!-- <a-tab-pane key="10" tab="Programs" v-if="role.user.staff.id!=paramId && (arrayToObjact(screensPermissions, 482) || arrayToObjact(screensPermissions, 483))">
                        <a-row :gutter="24" v-if="activeKey1=='10'">
                            ...
                            <a-col :sm="24" :xs="24" v-if="arrayToObjact(screensPermissions, 482)">
                                <StaffProgramsTable :hideAction="false"/>
                                <Loader />
                            </a-col>
                        </a-row>
                    </a-tab-pane> -->
...
<!-- <a-modal class="modalH-auto" width="40%" v-model:visible="visibleRole" title="Add Roles" :maskClosable="false" centered @cancel="roleCloseModal('visibleRole')" :footer="false"> -->
  <a-drawer placement="right" :closable="false" :visible="visibleRole" width="60%">
    <AddStaffRoleForm :clearData="clearData" :paramId="paramId" @onClose="roleCloseModal($event)" />
  </a-drawer>
<!-- </a-modal> -->
<!------>
...
import AddStaffRoleForm from ".././modals/forms/AddStaffRoleForm";
...
import { warningSwal,rolesPermissionFind } from "@/commonMethods/commonMethod";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      store.dispatch("staffAppoitnments", router.params.udid);
    ...
    const roleCloseModal = (val) => {
      visibleRole.value = val;

  ...
  <!-- <a-typography-title :level="3" class="common-sub-heading">Readings</a-typography-title> -->
    <a-table v-if="patientVitalsTable.length > 0" :dataSource="patientVitalsTable" :columns="columns" class="common-table" :pagination="false" :customRow="customRow">
      <template #bodyCell="{ column, record }">
        <!-- add asterisk for manual entry -->
        <!-- <template v-if="column.dataIndex === 'takeDate'">
              <span>{{ record.takeDate }}<span style='color:red' v-if="record.addType==='Manual'">*</span></span>
        </template> -->
    ...
    <a-drawer placement="right" :closable="false" :visible="visibleModal" width="60%">
      <ReviewVitals v-if="reviewVitalsModal" @hideDrawer="onClose" :deviceId="deviceIds.heartRateDeviceId" :vitalsId="vitalsId" activeTab="heartRate" />
      <VitalDetail v-else-if="vitalDetailsModal && visibleModal" @hideDrawer="onClose" :deviceId="deviceIds.heartRateDeviceId" :vitalsId="vitalsId" activeTab="heartRate" />
    </a-drawer>
    <Pagination v-if="patientVitalsTable?.length > 0" :meta="vitalsStore?.graphsListMeta" commit="vitals/patientVitalsTable" :endpoint="`patientData/${patientId}/vital`" :params="`&deviceType=${deviceIds.heartRateDeviceId}`" loader="vitals/patientVitalsLoader" entity="vitalsList" />
</template>

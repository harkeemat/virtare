  ...
  <!-- <a-typography-title :level="3" class="common-sub-heading">Readings</a-typography-title> -->
    <a-table v-if="patientVitalsTable.length > 0" :dataSource="patientVitalsTable" :columns="columns" class="common-table" :pagination="false" :customRow="customRow">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex == 'glucose'">
          <span v-if="record?.isDeleted" class="strike-through">{{ record?.glucose }}</span>
          <span v-else>{{ record?.glucose }}</span>
        </template>
    ...
    <a-drawer placement="right" :closable="false" :visible="visibleModal" width="60%">
      <ReviewVitals v-if="reviewVitalsModal" @hideDrawer="onClose" :deviceId="deviceIds.glucoseDeviceId" :vitalsId="vitalsId" activeTab="glucoseLevels" />
      <VitalDetail v-else-if="vitalDetailsModal && visibleModal" @hideDrawer="onClose" :deviceId="deviceIds.glucoseDeviceId" :vitalsId="vitalsId" activeTab="glucoseLevels"  />
    </a-drawer>
    <Pagination v-if="patientVitalsTable?.length > 0" :meta="vitalsStore?.graphsListMeta" commit="vitals/patientVitalsTable" :endpoint="`patientData/${patientId}/vital`" :params="`&deviceType=${deviceIds.glucoseDeviceId}`" loader="vitals/patientVitalsLoader" entity="vitalsList" />
</template>

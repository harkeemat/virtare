export default {
    gender:null,
    language:[],
    country:null,
    state:null,
    pmOfcontact:null,
    ptOfDayContact:null,
    healthCondition:null,
    relation:null,
    designations:null,
    programList:null,
    deviceType:null,
    program:null,
    timeline:null,
    taskPriority:[],
    network:null,
    specialization:[],
    taskCategory:[],
    successMsg:null, 
    insuranceName:null,
    insuranceType:null,
    documentTags:null,
    documentTypes:null,
    loadingStatus:false,
    duration:null,
    typeOfVisit:null,
    staffList:null,
    noteTypes:null,
    noteCategories:null,
    programCategories:null,
    vitalFieldsList:null,
    vitalFieldsByDeviceId:null,
    frequencyTypes:null,
    modules:null,
    // allPatientsList:null,
    allStaffList:null,
    activeCptCodes:null,
    checkChangeInput:false,
    getNotifications:null,
    searchTable:"&search=",
    notificationList:null,
    dropdownLoadingStatus:false,
    dropdownListing:null,
    loadingTableStatus:null,
    actionTrack:null,
    orderTable:{data:"&orderField=&orderBy="},
    getNotificationsList:null,
    questionnaireTemplateType:null,
    staffLevels:null,
    filter:null,
    removalReason:[],
    timeApprovalType:null,
    timeApprovalStatus:null,
    apiCounter:0,
    singleGlobalCode:[],
    tabGlobalCode:[],
    questionnaireLoadingStatus:'',
    dashboardLoadingStatus:'',
    logoutLoader:'',
    bugCategory:null,
    tooltip:[],
    formTitle:[],
    manufacturer:[],
    connectivity:[],
    contractType:[],
    // phone msaking objects
    bindProps:{
        mode:"national",
        defaultCountry: "US",
        disabledFetchingCountry: true,
        disabled: false,
        disabledFormatting: true,
        placeholder: "Enter a phone number",
        required: true,
        enabledCountryCode: false,
        enabledFlags: true,
        autocomplete: "off", 
        onlyCountries:['US'],
        
    },
    questionDataType:null,
    workflowEventType:null,
    workFlowAction:null,
    toScroll:false,
    dateFilter:null,
    taskType:null,
    taskStatus: [],
    otherFilters:null,
    CPTCodeStatus:null,
    showStartTimer:false,
    showPauseTimer:true,
    showResumeTimer:false,
    loadingTimelineStatus:false,
    messageType:null,
    escalationAction:[],
    escalationType:[],
    locationSelected:[],
    componentKey:0,
    notificationsMeta:null,
    filterDateDataGet:[],
    clientStatusCodes:[],
    timeZone:[],
    rolesDropDown:[],
    loading:false,
    patientActionTimerStatus: "", // running, paused, stopped
    patientActionTimerValue: null,
    timeLogDepartments: [],
    timeLogType: [],
    timeLogContractType: [],
    getClientsForPatients:[],
    windowUpdated:false,
    patientActionTime:null,
}